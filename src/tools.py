...
async def delegate_job_application(job_details: dict) -> str:
    """Tool used by the Manager LLM to delegate a job to a subagent.
    ...
    target_worker_model = worker_model_holder.get("model")
    if target_worker_model is None:
        raise RuntimeError("worker_model_holder is empty — initialize_tools() was not called before delegating.")

    # Debug: print bound tool count so we can verify MCP tools are present
    bound_tools = target_worker_model.kwargs.get('tools', [])
    console.print(f"[dim]Subagent worker_model has {len(bound_tools)} bound tools[/dim]")
    ...
    console.print(
        Panel(
            ...
            title=f"🤖 [bold green]Subagent Spawned — Job {job_id}[/bold green]",
            border_style="cyan"
        )
    )
    ...
    # Run directly without console.status so logs print live
    ...
    console.print(f"[bold green]✅ Subagent Finished Job {job_id}:[/bold green] {last_message}\n")
...
def get_pending_jobs() -> list[dict]:
    ...
    if "application_status" not in df.columns:
        ...
        pending = df[df["application_status"].isin(["Not Applied", "In Progress", "Failed"])].copy()
